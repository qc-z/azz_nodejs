...
UsercouponSchema.virtual('token').get(function() {
...
UsercouponSchema.pre('save', function(next) {
  ...
  // }
  if (!usercoupon.isModified('password')) return next()

  bcrypt.genSalt(SALT_WORK_FACTOR, function(err, salt) {
    if (err) return next(err)
    bcrypt.hash(usercoupon.password, salt, function(error, hash) {
      if (error) return next(error)

      usercoupon.password = hash
      next()
    })
  })
})
